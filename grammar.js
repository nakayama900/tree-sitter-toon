...
module.exports = grammar({
  ...
  rules: {
    ...
    document: ($) => choice($.object, $.array, $.value),
    ...
    object: ($) =>
      choice(
        ...
        seq($._indent, repeat($.pair), $._dedent),
      ),
    ...
    pair: ($) =>
      seq(
        ...
        choice(
          ...
          seq(":", optional(/[ \t]+/), field("value", $.value), $._newline),
          // Pair without header - nested object
          seq(":", $._newline, field("value", $.object)),
        ),
      ),
    ...
    row: ($) => choice(prec(3, $.value_row), prec(1, $.object_row)),

    value_row: ($) => seq(token(seq("-", /[ \t]+/)), $.row_values, $._newline),

    row_values: ($) => choice($.single_value, $.delimited_values),
    ...
    object_with_first_field: ($) =>
      prec.left(
        seq(
          ...
          optional($.list_object_continuation),
        ),
      ),
    // indented_object_body: ($) => seq($._indent, repeat1($.pair), $._dedent),
    list_object_continuation: ($) =>
      seq(
        $._indent,
        repeat($.pair), // PoC-Fで導入した repeat を使用し、0個以上のペアを許可
        $._dedent,
      ),
    ...
    // Values
    value: ($) =>
      choice($.string, $.null, $.boolean, $.number, $.unquoted_string, alias($.unquoted_key, $.unquoted_string)),
